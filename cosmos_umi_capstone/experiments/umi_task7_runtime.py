...
class FeedbackRuntime:
    ...
    def step(self, condition_only: Any, step_index: int) -> dict[str, Any]:
        if isinstance(step_index, bool) or not isinstance(step_index, numbers.Integral) or int(step_index) not in (0, 1):
            raise ValueError("step_index must be exactly 0 or 1")
        ...
        try:
            ...
            request_seed = int(step_index)  # the approved paired seeds are exactly [0, 1]
