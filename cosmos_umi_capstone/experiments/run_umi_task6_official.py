...
try:
    from .umi_task6_operational import (OfficialRuntimeFactory, OperationalEvidenceError, extract_task5_directions,
        prepare_bridge_upload_bundle, validate_launch_contract, verify_pinned_bridge_assets)
    from .umi_task6_primitives import STATE_CATALOG
    from .umi_task6_runtime import Task6Inputs, preflight_task6
    ...
    from umi_task6_operational import OfficialRuntimeFactory, OperationalEvidenceError, extract_task5_directions, prepare_bridge_upload_bundle, validate_launch_contract, verify_pinned_bridge_assets
    from umi_task6_primitives import STATE_CATALOG
    from umi_task6_runtime import Task6Inputs, preflight_task6
    from run_umi_task6_experiment import BlockedExecution, ResourceMonitor, accept_resource_smoke, run_pilot, run_resource_smoke
    from umi_task6_decoder import run_task6_decoder_replays
...
def execute_official(args: argparse.Namespace) -> dict[str, Any]:
    ...
    observed = load_json(args.observed_evidence)
    validate_launch_contract(contract, observed=observed)
    ...
    if args.phase == "preflight":
        ...
        result = {"status": "PREFLIGHT_COMPLETE", "phase": "RESOURCE_SMOKE", "generation_started": False,
                  "group": dict(contract["group"]), "asset_evidence": assets,
                  "task5_evidence": {k: v for k, v in task5.items() if k != "bank"}}
        (root / "run_status.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        return result
    ...
    if generation.get("status") not in {"AWAITING_REVIEW", "COMPLETE"}:
        return generation
    if encoder is None: raise OperationalEvidenceError("official loader must expose a condition encoder for decoder replay")
    ...
    if decoder.get("status") != "COMPLETE": return decoder
...
def parse_args(argv=None):
    ...
    parser.add_argument("--launch-contract", required=True); parser.add_argument("--observed-evidence", required=True)
